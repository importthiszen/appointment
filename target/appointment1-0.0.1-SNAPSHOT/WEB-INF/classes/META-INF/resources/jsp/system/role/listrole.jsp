...
<html>
				...
				<div class="main-content-inner">
					<div class="page-content">
						<div class="row">
							...
							<!-- 检索  -->
							<form  method="post" name="infoForm" action="/sys/listRole" >
								<div class="container">
									...
									SysRole firstObj = (SysRole)request.getAttribute("firstObj");
										if(firstObj!=null){
											...
											for(int i = 0; i < list.size(); i++){
												SysRole temRole = list.get(i);
												if(firstId.equals(temRole.getRoleId())){
													//如果增加或修改的数据本身就在第一页，调整增加的数据到第一位
													SysRole role0 = list.get(0);
													list.set(0, temRole);
													list.set(i, role0);
													flag = true;
												}
										...
										<c:when test="${not empty page.list}">
											<c:forEach items="${page.list}" var="role" varStatus="i">
												<tr>
													...
													<td align="center">
														<c:if test="${role.roleState==\"A\"}">可用</c:if>			
														<c:if test="${role.roleState==\"N\"}">不可用</c:if>
													</td>
								...
								<div align="center">
									<yzpage:page pageName="page" url="/sys/listRole"></yzpage:page>
								</div>
							</form>
							</div>
						</div>
	...
	function editpage(id){
		location.href = "/sys/pageRole?id="+id;
	}
	function addpage(){
		location.href = "/sys/pageRole";
	}
	function auth(id){
	...
	function delOne(id){
		bootbox.confirm("确定要删除吗?", function(result) {
			if(result){
				location.href = "/sys/removeRole?ids="+id;
			}
		})
	}
	
	function doSearch(){
		$("form[name='infoForm']").attr("action","/sys/listRole");
		...
		bootbox.confirm("'确定要删除选中的数据吗?", function(result) {
			if(result) {
				...
				if(checked.length==0){
					...
					$("form[name='infoForm']").attr("action","/sys/removeRole");
					$("form[name='infoForm']").submit();
				}
			}
		});
	...
	//对用户进行授权
	function auth(id){
		location.href = "/sys/authRole?id="+id;
	}
	</script>
</html>