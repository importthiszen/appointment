				...
				<div class="container" >
					...
					<div class="form-group" >
						...
						<div class="col-md-7 col-sm-7">
							<input class="form-control" name="bigthingTitle"  value="" oninput="validatName()" placeholder="请输入大事件标题" />
						</div>	
					</div>
					<div class="form-group">
...
// 		if (bigthingTitle == "${anno.noticeTitle}") {
// 			return ;
// 		}
		if(bigthingTitle != ""){
			$.ajax({
	...
	function doSub(){
		...
		bootbox.confirm("你确定要保存吗?",function(r){
			if (r) {
				var url = "<%=request.getContextPath()%>/baseinfo/addBigThing.action";
				var data = $("form[name='infoForm']").serialize();
				//post请求     没有缓存     异步   没有error
				$.post(url, data, function(r) {
					if (r.success) {
						location.href="<%=request.getContextPath()%>/baseinfo/listBigthing.action?pageNum=1";
					} else {
						bootbox.alert("添加失败!");
					}
				}, "json")			
