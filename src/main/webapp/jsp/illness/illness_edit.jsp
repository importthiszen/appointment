				...
				<div class="container ">					
	               <div class="form-group">
						...
						<div class="col-md-6 col-sm-6">
							<select name="illtpId" class="form-control" onchange="gaibian()">
								...
								<c:forEach items="${illtpList}" var="illtp">
									<option value="${illtp.illtpId}"${illtp.illtpId==illness.illtpId?"selected":"" }>${illtp.illtpName}</option>
								</c:forEach>
							</select>
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-md-6 col-sm-6">
							<input class="form-control" name="illName" value="${illness.illName}" oninput="validatName()" placeholder="请输入疾病名称" />
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-sm-3 col-md-2">疾病简介:</label>
						<div class="col-md-6 col-sm-6">
							<textarea class="form-control" name="illInfo" rows="6"  placeholder="请输入疾病简介" >${illness.illInfo}</textarea>
						</div>
				    </div>
					<div class="form-group">
	...
	if(bj){
	 ...
	 bootbox.confirm("确认提交吗?",function(r){
		 if(r){
			 $.ajax({
				type:"POST",
				url:"<%=request.getContextPath()%>/illness/editsave.action",
